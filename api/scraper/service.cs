    ...
    public class ScraperService : IScraperService
    {
        ...
        public async Task<object> scraperTiktok(TikTokProfileRequest item, string idUser)
        {
            try
            {
                ...
                var cekStatus = await _scraperCollection.Find(_ => _.IdUser == idUser).FirstOrDefaultAsync();
                if (cekStatus != null)
                {
                    cekStatus.Follower = firstVideo;
                    cekStatus.UpdatedAt = DateTime.Now;
                    await _scraperCollection.ReplaceOneAsync(_ => _.IdUser == idUser, cekStatus);
