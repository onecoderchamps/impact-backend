...
namespace Trasgo.Server.Controllers
{
    ...
    public class ScraperController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<object> GetbyUser()
        {
            try
            {
                ...
                var data = await _IScraperService.GetById(idUser);
                return Ok(data);
            }
            catch (CustomException ex)
            {
        ...
        public async Task<object> GetbyUser([FromRoute] string category)
        {
            try
            {
                var claims = User.Claims;
                ...
                string accessToken = HttpContext.Request.Headers["Authorization"];
                string idUser = await _ConvertJwt.ConvertString(accessToken);
                var data = await _IScraperService.GetFromCategory(idUser, category);
                return Ok(data);
            }
            catch (CustomException ex)
            {
